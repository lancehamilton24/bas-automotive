...
export function ContactForm() {
  ...
  return <form className="contact-form" action={`https://formsubmit.co/${contactEmail}`} method="POST" aria-labelledby="contact-form-title">
    ...
    <div className="contact-form-row">
      <label htmlFor="contact-name">Name (required)<input id="contact-name" name="name" autoComplete="name" required maxLength={100} pattern=".*\S.*" title="Please enter your name, not just spaces." /></label>
    ...
    <label htmlFor="contact-email">Email (optional)<input id="contact-email" name="email" type="email" autoComplete="email" maxLength={254} pattern="[^\s@]+@[^\s@]+" title="Please enter a valid email address containing @, such as name@example.com." /></label>
